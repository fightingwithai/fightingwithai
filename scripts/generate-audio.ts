...
/**
 * Generate audio files from content using Piper TTS.
 *
 ...
 * 6. Generates WAV audio with numeric prefixes (01-, 02-, etc.)
 * 7. Updates manifest
 */
...
import { generateAudio, checkPiperAvailable } from './audio/piper-wrapper.js';
...
function getCollectionFiles(config: CollectionConfig): OrderedContentFile[] {
  ...
  // Add order and filename
  return sorted.map((file, index) => ({
    ...file,
    order: index + 1,
    filename: `${String(index + 1).padStart(2, '0')}-${file.slug}.wav`,
  }));
}
...
async function main(): Promise<void> {
  console.log(`\n${CYAN}🎙️  Piper TTS Audio Generator${RESET}\n`);

  // Check Piper is available
  if (!checkPiperAvailable()) {
    console.error(`${RED}Piper TTS not available. Aborting.${RESET}`);
    process.exit(1);
  }
